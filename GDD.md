...
- Set-size floor. A source word may headline a day only if its set has at least MIN_SET_SIZE words, counting the crown, the same crown-inclusive count the "X of Y" counter shows. MIN_SET_SIZE is 15. Sub-floor words are not deleted; they stay real words found inside other racks, they just never headline. The floor keeps a daily from ever being thin, and it is computed through the engine's createPuzzle, never reimplemented.
- The frozen daily calendar. The daily sequence is a committed, append-only artifact (public/data/daily-calendar.json): an ordered list of eligible source words plus a fixed epoch date. Day N is the word at position N within the first cycle. The calendar is generated by scripts/build-calendar.ts, reusing createPuzzle for eligibility. After the pool is exhausted, each later cycle is a deterministic reshuffle, and the first word of a new cycle never equals the last word of the previous one.
- APPEND-ONLY INVARIANT (load-bearing, do not break): the daily calendar is append-only. Never reorder it, never remove from it. New words go on the end. Removing or reordering an entry re-dates every day after it and breaks the promise that a given day is a fixed puzzle. This is what makes future pool expansion a painless append rather than a reshuffle. The first establishment was a one-time, intended reshuffle; it is the last one.
- Endless: a fresh puzzle on demand, from the New Puzzle button. Does not touch the streak. Endless draws at random from the same eligible calendar words, so a sub-floor word never headlines endless either.
